...
class CNNandDinov2(nn.Module):
    def __init__(self, cnn_kwargs = None, amp = False, dinov2_weights = None, amp_dtype = torch.float16):
        ...
        if dinov2_weights is None:
            dinov2_weights = torch.hub.load_state_dict_from_url("https://huggingface.co/datasets/Liangyingping/L2Mpp-checkpoints/resolve/main/dinov2l_distilled.pth", map_location="cpu")
            if isinstance(dinov2_weights, dict) and ("model" in dinov2_weights or "state_dict" in dinov2_weights):
                dinov2_weights = dinov2_weights.get("model", dinov2_weights.get("state_dict"))

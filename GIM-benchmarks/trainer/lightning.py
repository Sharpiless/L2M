...
class Trainer(pl.LightningModule):
    ...
    def __init__(self, pcfg, tcfg, dcfg, ncfg):
        ...
        elif pcfg.weight == 'l2mpp':
            import sys
            sys.path.append("/data6/liangyingping/git-code/L2M/")
            from romatch import l2mpp_model
            detector = None
            ckpt2load = None
            # if not ckpt2load is None:
            #     print("loading:", ckpt2load)
            #     ckpt2load = torch.load(ckpt2load, map_location="cpu")
            # 
            model = l2mpp_model(device='cpu', coarse_res = 560, upsample_res = 864, weights=ckpt2load)
            # model = l2mpp_model(device='cpu', weights=ckpt2load)
        ...
        if pcfg.weight == 'l2mpp' and pcfg.version == 'official':
            return
        print("[checkpoints_path] loading:", checkpoints_path)
    ...
    def inference(self, data):
        ...
        elif self.pcfg.weight == 'l2mpp':
            self.gim_l2mpp_inference(data)
    ...
    def gim_l2mpp_inference(self, data):
        dense_matches, dense_certainty = self.model.match(data['img_path0'][0], data['img_path1'][0])
        sparse_matches, mconf = self.model.sample(dense_matches, dense_certainty, 5000)
        # sparse_matches, mconf = self.model.sample(dense_matches, dense_certainty)

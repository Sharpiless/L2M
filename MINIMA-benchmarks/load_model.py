...
def load_l2mpp(args, test_orginal_megadepth=False):
    import sys
    ...
    from src.utils.data_io_l2mpp import DataIOWrapper, lower_config
    config = get_cfg_defaults(inference=True)
    config = lower_config(config)
    device = torch.device('cuda:{}'.format(0) if torch.cuda.is_available() else 'cpu')
    if args.ckpt is not None:
        print("[loading]:", args.ckpt)
        ckpt2load = torch.load(args.ckpt, map_location="cpu")
    else:
        ckpt2load = None
    matcher = l2mpp_model(device=device, weights=ckpt2load, coarse_res = 560, upsample_res = 864)
    # matcher = l2mpp_model(device=device, weights=ckpt2load)
    matcher.eval()
...
def choose_method_arguments(parser):
    parser.add_argument('--method', type=str, required=True,
                        choices=["xoftr", 'sp_lg', 'loftr', 'roma', 'l2mpp'],

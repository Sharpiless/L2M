...
def test_relative_pose_vistir(
    ...
    if method == "roma" or method == "l2mpp":
    ...
    # 2) 平均：基于“所有npz scene”（scene_pose_auc / precs）
    ...
    # 写入 results（key 名字你也可以按自己习惯改）
    results["avg_auc_over_agg_scenes"] = (100.0 * (avg_auc_over_agg_scenes / 100.0)).tolist() \
        if np.isfinite(avg_auc_over_agg_scenes).all() else avg_auc_over_agg_scenes.tolist()
    # 上面这一行其实等价于 avg_auc_over_agg_scenes.tolist()，保留只是为了强调单位一致（你现在是 100*auc）

    results["avg_auc_over_all_npz_scenes"] = (100.0 * (avg_auc_over_all_npz_scenes / 100.0)).tolist() \
        if np.isfinite(avg_auc_over_all_npz_scenes).all() else avg_auc_over_all_npz_scenes.tolist()
